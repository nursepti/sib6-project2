...
-- 1
SELECT SUM(revenue) AS total_revenue_Q1_2024
FROM data_warehouse.sales_fact sf
left JOIN data_warehouse.time_dimension td ON sf.date_id = td.date_id
WHERE td.quarter = '1' AND td.year = 2024;

-- 2
SELECT SUM(quantity) AS total_quantity_Q4_2023
FROM data_warehouse.sales_fact sf
left JOIN data_warehouse.time_dimension td ON sf.date_id = td.date_id
WHERE td.quarter = '4' AND td.year = 2023;

-- 3
SELECT td.quarter, td.year, SUM(sf.revenue) AS total_revenue_tertinggi
FROM data_warehouse.sales_fact sf
left JOIN data_warehouse.time_dimension td ON sf.date_id = td.date_id
